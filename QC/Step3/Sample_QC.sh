...
# 确保样本QC文件存在
if [ ! -f "./sample_qc_final_keep.txt" ]; then
...
for i in {1..22}; do
    ...
    --make-bed \
    --threads 30 \
    --out ./QCStep3_v2/ukb_wes_step3_chr${i}
    ...
    if [ $? -eq 0 ]; then
        ...
        # 检查生成的文件是否存在
        if [ -f "./QCStep3_v2/ukb_wes_step3_chr${i}.bed" ] && [ -f "./QCStep3_v2/ukb_wes_step3_chr${i}.bim" ] && [ -f "./QCStep3_v2/ukb_wes_step3_chr${i}.fam" ]; then
            echo "Output files generated successfully."
            
            # 上传结果
            echo "Uploading chromosome ${i} files..."
            dx upload -r ./QCStep3_v2/ukb_wes_step3_chr${i}.* --destination "project-GYXj5k0JzYzPbyBYx8V7BFg3:/Jzhang_data/exome_data/qc_step3/"
            ...
            if [ $? -eq 0 ]; then
                ...
                rm ./QCStep3_v2/ukb_wes_step3_chr${i}.*
                echo "Files removed."
            else
...
# 检查是否有任何文件剩余
remaining_files=$(ls ./QCStep3_v2/ | wc -l)
if [ $remaining_files -eq 0 ]; then
    echo "All files have been processed, uploaded, and removed successfully."
else
    echo "Warning: $remaining_files files remain in the QCStep3_v2 directory."
    ls -lh ./QCStep3_v2/
fi
